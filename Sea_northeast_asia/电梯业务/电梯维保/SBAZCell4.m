...
- (BOOL)configTableViewCell:(id)aObject_entity index_row:(int)aIndex_row count:(int)aCount_source;
{
    ...
    _labAddr.text=warnmodel.Lift.InstallationAddress;
     _labStatus.text=[NSString stringWithFormat:@"维保状态：%@", warnmodel.TotalLossTime];
    //2.1CreateTime 并且 计算totalTime
    NSArray *Time = [warnmodel.CreateTime componentsSeparatedByString:@"T"];
    NSArray *cTime = [warnmodel.UseConfirmTime componentsSeparatedByString:@"T"];
   _labDate.text=[NSString stringWithFormat:@"%@--%@", [Time objectAtIndex:0],[cTime objectAtIndex:0]];
