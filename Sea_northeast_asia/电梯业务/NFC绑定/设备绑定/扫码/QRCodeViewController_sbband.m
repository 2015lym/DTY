...
#import "QRCodeViewController_sbband.h"
#import <AVFoundation/AVFoundation.h>
#import "CommonUseClass.h"

#import "MyControl.h"
@interface QRCodeViewController_sbband ()
...
@implementation QRCodeViewController_sbband
@synthesize app;
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if (metadataObjects.count > 0) {
        ...
        LiftNum = metadataObject.stringValue;
...
-(void)gotoDetail
{
    ...
    [dicHeader setValue:LiftNum forKey:@"deviceNum"];
    [dicHeader setValue:app.userInfo.UserID forKey:@"UserId"];
    NSString *url=[NSString stringWithFormat:@"Equipments/GetLiftEequipment?deviceNum=%@",LiftNum];
    [XXNet GetURL:url header:nil parameters:dicHeader succeed:^(NSDictionary *data) {
        ...
        if ([[CommonUseClass FormatString: data[@"Message"]] isEqual:@"操作成功"]) {
            ...
            if(arrData.count>0)
            {
                ...
                UploadDate=[CommonUseClass FormatString:dic [@"CertificateNum"]];
                ...
                liftID=@"have";
                LiftNum=[dic objectForKey:@"DeviceNum"];
        ...
        else if ([[CommonUseClass FormatString: data[@"Message"]] isEqual:@"设备序列号错误"]) {
            [self performSelectorOnMainThread:@selector(dd:) withObject:@"设备序列号错误" waitUntilDone:YES];
        }
        else
        {
            NSString *str_json = data [@"Data"];
...
- (void)gotoDetail_go{
    ...
    SBBandSaveViewController *ctvc=[[SBBandSaveViewController alloc] init];
    ctvc.Title=@"一键通绑定";
    ctvc.liftNum=LiftNum;
    ctvc.liftID=liftID;
    ctvc.InstallationAddress=InstallationAddress;
    ctvc.UploadDate=UploadDate;
    ctvc.nfcList=nfcList;
