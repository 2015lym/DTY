        ...
        [self.motionManager startAccelerometerUpdatesToQueue:[NSOperationQueue currentQueue] withHandler:^(CMAccelerometerData *accelerometerData, NSError *error)
         {
             ...
             if ((fabs(y) + 0.1f) >= fabs(x))
             {
                 ...
                 if (y >= 0.1f)
                 {
                     // Down
                     NSLog(@"Down");
                     self.shootingOrientation = UIDeviceOrientationPortraitUpsideDown;
                 }
                 else
                 {
                     // Portrait
//                     NSLog(@"Portrait");
                     self.shootingOrientation = UIDeviceOrientationPortrait;
                 }
             }
             else
             {
                 ...
                 else if (x <= 0.1f)
                 {
                     // Left
                     NSLog(@"Left");
                     self.shootingOrientation = UIDeviceOrientationLandscapeLeft;
                 }
                 else
                 {
                     // Portrait
