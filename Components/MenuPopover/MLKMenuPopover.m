...
#define MENU_POINTER_RECT       CGRectMake(frame.origin.x+55, frame.origin.y, 23, 11)
...
- (id)initWithFrame:(CGRect)frame menuItems:(NSArray *)aMenuItems selectRow:(int)selectRow
{
    ...
    if (self)
    {
        ...
        [self.containerButton addTarget:self action:@selector(btn_containerButton) forControlEvents:UIControlEventTouchUpInside];
        ...
        bgView.layer.cornerRadius = 8; //设置图片圆角的尺度
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.selectRow==indexPath.row) {
        [cell.textLabel setTextColor:[UIColor colorWithRed:255.f/255.f green:156.f/255.f blue:0.f/255.f alpha:1]];
    }
...
{
    [self.menuPopoverDelegate containerButton_OnClick];
}
