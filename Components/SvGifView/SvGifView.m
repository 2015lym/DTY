...
- (id)initWithCenter:(CGPoint)center fileURL:(NSURL*)fileURL;
{
    ...
    if (self) {
        NSArray *array_paths=fileURL.pathComponents;
        strImageName=[[NSString alloc]initWithFormat:@"%@",[array_paths objectAtIndex:array_paths.count-1] ];
        ...
        self.frame = CGRectMake(0, 0, _width, _width);
        self.center = center;
    }
...
- (void)startGif
{
    ...
    if (isPlay==YES) {
        ((UIImageView*)self.superview).image=[UIImage imageNamed:nil];
        return;
    }
    ((UIImageView*)self.superview).image=[UIImage imageNamed:nil];
    isPlay=YES;
    ((UIImageView*)self.superview).image=nil;
    ...
    for (int i = 0; i < count; ++i) {
        [times addObject:[NSNumber numberWithFloat:(currentTime / _totalTime-0.1f)]];
...
- (void)stopGif
{
    ...
    //((UIImageView*)self.superview).image=imageBack;
}
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag
{
    ...
    {
        ((UIImageView*)self.superview).image=[UIImage imageNamed:strImageName];
    }
