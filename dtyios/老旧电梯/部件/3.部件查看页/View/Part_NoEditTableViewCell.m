...
@implementation Part_NoEditTableViewCell
...
+ (instancetype)cellWithTableView:(UITableView *)tableview {
    static NSString *cellID = @"Part_NoEditTableViewCell";
    Part_NoEditTableViewCell *cell = [tableview dequeueReusableCellWithIdentifier:cellID];
    if (!cell) {
        cell = (Part_NoEditTableViewCell *)[[[NSBundle mainBundle] loadNibNamed:cellID owner:self options:nil] lastObject];
    }
    return cell;
}

- (IBAction)editReason:(id)sender {
    if ([self.delegate respondsToSelector:@selector(cellEditAction:andIndexPath:)]) {
        [self.delegate cellEditAction:edit_reason andIndexPath:_indexPath];
    }
}

- (IBAction)editOut:(id)sender {
    if ([self.delegate respondsToSelector:@selector(cellEditAction:andIndexPath:)]) {
        [self.delegate cellEditAction:edit_out andIndexPath:_indexPath];
    }
}
