//
//  Task_RiskPartDetailTableViewCell.m
//  Sea_northeast_asia
//
...
@implementation Task_RiskPartDetailTableViewCell
...
+ (instancetype)cellWithTableView:(UITableView *)tableview {
    static NSString *cellID = @"Task_RiskPartDetailTableViewCell";
    Task_RiskPartDetailTableViewCell *cell = [tableview dequeueReusableCellWithIdentifier:cellID];
    if (!cell) {
        cell = (Task_RiskPartDetailTableViewCell *)[[[NSBundle mainBundle] loadNibNamed:cellID owner:self options:nil] lastObject];
    }
    return cell;
}
...
- (void)setModel:(ElevatorAssessmentItemEntityList *)model {
    ...
    if ([[NSUserDefaults standardUserDefaults] objectForKey:[NSString stringWithFormat:@"riskpart_%@_%@", _taskId, model.ItemId]]) {
        _partInfoLabel.text = @"已填写";
    } else {
        _partInfoLabel.text = @"未填写";
...
- (IBAction)partInfoAction:(id)sender {
    if ([self.delegate respondsToSelector:@selector(selectAction:andIndexPath:)]) {
        [self.delegate selectAction:PartInfo andIndexPath:_indexPath];
    }
}
