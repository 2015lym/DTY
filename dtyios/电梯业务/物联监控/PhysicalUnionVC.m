...
@implementation PhysicalUnionVC
@synthesize app;
- (void)viewWillAppear:(BOOL)animated {
    [super viewWillAppear:animated];
    self.navigationItem.title=@"物联监控";
...
- (void)viewDidLoad {
    ...
    CourseTableview=[[UITableViewExViewController alloc]initWithStyle:UITableViewStylePlain tableCellXIB:@"PhysicalUnionCell" tableCells_Index:0];
    //代理
    CourseTableview.delegateCustom=self;
    //行高
    CourseTableview.currHeight= 50;
    //tableView的frame
    CourseTableview.tableView.frame = CGRectMake(0, CGRectGetMaxY(line.frame), SCREEN_WIDTH, SCREEN_HEIGHT-CGRectGetMaxY(line.frame));
...
-(void)initMySelect
{
    ...
    mySelectView.view.frame = CGRectMake(0, 40, bounds_width.size.width,bounds_width.size.height);
...
- (void)RequestNetData {
    ...
    [XXNet GetURL:GetProblemListURL header:dicHeader parameters:nil succeed:^(NSDictionary *data) {
