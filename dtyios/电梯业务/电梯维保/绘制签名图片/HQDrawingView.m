...
//    self.cancleArr = [NSMutableArray new];
    ...
    //if (self) {
        ...
        UIPanGestureRecognizer *pan=[[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(writeLineName:)];
...
- (CGPoint) midpoint:(CGPoint) p0 for1:( CGPoint) p1 {
    return (CGPoint) {
        (p0.x + p1.x) / 2.0,
        (p0.y + p1.y) / 2.0
    };
...
-(void)writeLineName:(UIPanGestureRecognizer *)pan
{
    CGPoint current=[pan locationInView:self];
    CGPoint midP=[self midpoint:previous for1:current];
    if (pan.state==UIGestureRecognizerStateBegan)
    {
        [self.bezier moveToPoint:current];
    }
    else if (pan.state==UIGestureRecognizerStateChanged)
    {
        [self.bezier addQuadCurveToPoint:midP controlPoint:current];
    }
    previous=current;
