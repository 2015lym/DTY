...
- (BOOL)configTableViewCell:(id)aObject_entity index_row:(int)aIndex_row count:(int)aCount_source;
{
    ...
    if(![[CommonUseClass FormatString:warnmodel.TotalLossTime ] isEqual:@""])
    {
     if([warnmodel.TotalLossTime rangeOfString:@"安装"].location ==NSNotFound)
        _labStatus.text=[NSString stringWithFormat:@"联网状态：%@", warnmodel.TotalLossTime];
     else
        _labStatus.text=[NSString stringWithFormat:@"设备状态：%@", warnmodel.TotalLossTime];
    }
