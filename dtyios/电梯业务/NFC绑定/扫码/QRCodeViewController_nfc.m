//
//  QRCodeViewController_nfc.m
//  Sea_northeast_asia
//
//  Created by 邓玉 on 2018/7/9.
//  Copyright © 2018年 SongQues. All rights reserved.
//
#import "QRCodeViewController_nfc.h"
#import <AVFoundation/AVFoundation.h>
#import "CommonUseClass.h"

#import "MyControl.h"
@interface QRCodeViewController_nfc ()
...
@implementation QRCodeViewController_nfc
...
//    //test
    ...
    [dicHeader setValue:app.userInfo.UserID forKey:@"UserId"];
        NSString *url=[NSString stringWithFormat:@"NFC/GetCheckTermIsNFC?LiftNum=%@",LiftNum];
    [XXNet GetURL:url header:nil parameters:dicHeader succeed:^(NSDictionary *data) {
        ...
        if ([[CommonUseClass FormatString: data[@"Message"]] isEqual:@"操作成功"]) {
            ...
            if(arrData.count>0)
            {
                ...
                liftID=[dic objectForKey:@"LiftID"];
                nfcList=arrData;
                [self gotoDetail_go];
            }
        ...
        else
        {
            //[CommonUseClass showAlter:data[@"Message"]];
                NFCSaveViewController *ctvc=[[NFCSaveViewController alloc] init];
                ctvc.Title=@"NFC标牌绑定";
...
//    self.questionBox = [[WYXCSaveViewController alloc] initWithItem:self.item andConfigure:configure];
//    self.questionBox.typeStr=@"0";
//    self.questionBox.liftNum = LiftNum;
//    int tag=0;
//    self.questionBox.floorNumber=tag;
//    if([UploadDate isEqual:@""])UploadDate=@"尚未巡查";
//    self.questionBox.UploadDate=UploadDate;
//    self.questionBox.InstallationAddress=InstallationAddress;
//    self.questionBox.allTags=stet;
//    self.questionBox.PropertyCheckId=PropertyCheckId;
