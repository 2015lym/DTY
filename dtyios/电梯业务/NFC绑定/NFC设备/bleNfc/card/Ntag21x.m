...
-(void)NdefTextWrite:(NSString *)text callbackBlock:(onReceiveNdefTextWriteListener)block{
    ...
    NSData *writeData = [text dataUsingEncoding:NSUTF8StringEncoding];
    Byte NDEFTextByte[] = {0x03,(Byte)0xE8,(Byte)0xD2, 0x0A, 0x00, 0x74, 0x65, 0x78, 0x74, 0x2f, 0x70, 0x6c, 0x61, 0x69, 0x6e};
    NDEFTextByte[1] = (Byte)(writeData.length + 13);
    NDEFTextByte[4] = (Byte)writeData.length;
    NSMutableData *NDEFData = [[NSMutableData alloc] initWithBytes:NDEFTextByte length:15];
    [NDEFData appendData:writeData];
    NDEFTextByte[0] = (Byte)0xFE;
    [NDEFData appendBytes:NDEFTextByte length:1];
...
-(void)NdefTextReadWithCallbackBlock:(onReceiveNdefTextReadListener)block {
    ...
    @try {
        NSData *returnData = [self ultralightRead:4];
        if ( (returnData != nil) && (returnData.length != 16) ) {
        ...
        NSLog(@"读出数据为:%@", returnData);
        
        if ( (returnBytes[0] == 0x03) || (returnBytes[1] == 0x03) ) {
            int i, j;
            Byte imageBytes[] = {0x74, 0x65, 0x78, 0x74, 0x2f, 0x70, 0x6c, 0x61, 0x69, 0x6e};
            Boolean searchFlag = NO;
            for (i=0; i < 15; i++) {
                searchFlag = YES;
            ...
            if (searchFlag) {
                ...
                int textLen;
                if ( (imageStartAddr > 4) && (returnBytes[i-3] == 0) && (returnBytes[i-4] == 0x00) ) {
                ...
                returnData = [self ntag21xLongRead:4 endAddress:recordEndAddress];
                if ( (returnData == nil) || (returnData.length < recordLen) || (returnData.length < textLen + imageStartAddr + 10)) {
                    if (mOnReceiveNdefTextReadListenerBlock != nil) {
                        mOnReceiveNdefTextReadListenerBlock(NO, @"Read card fail", nil);
                    }
                    return;
                ...
                returnBytes = (Byte *)[returnData bytes];
                NSString* text = [[NSString alloc] initWithBytes:&returnBytes[imageStartAddr + 10] length:textLen encoding:NSUTF8StringEncoding];
                if (mOnReceiveNdefTextReadListenerBlock != nil) {
                ...
                if (mOnReceiveNdefTextReadListenerBlock != nil) {
                    mOnReceiveNdefTextReadListenerBlock(NO, @"Read card err", nil);
                }
            }
        }
        else {
