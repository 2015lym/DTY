//
//  MaintenancePhotoTableViewCell.m
//  多张照片选择简单DEMO
//
...
@interface MaintenancePhotoTableViewCell()<UINavigationControllerDelegate, UIImagePickerControllerDelegate, TZImagePickerControllerDelegate>

@end

@implementation MaintenancePhotoTableViewCell
...
+ (instancetype)cellWithTableView:(UITableView *)tableview {
    static NSString *cellID = @"MaintenancePhotoTableViewCell";
    MaintenancePhotoTableViewCell *cell = [tableview dequeueReusableCellWithIdentifier:cellID];
    if (!cell) {
        cell = (MaintenancePhotoTableViewCell *)[[[NSBundle mainBundle] loadNibNamed:cellID owner:self options:nil] lastObject];
    }
    return cell;
}
...
- (void)setModel:(AppMaintenanceItemDtos *)model {
    ...
    for (AppMaintenanceWorkRecordImgDtos *imgModel in model.photos) {
...
- (void)changeUI {
    ...
    for (int i=0; i<_dataArray.count; i++) {
        ...
        btn.tag = 20000 + i;
        if (_isPreview || [dic[@"ImgUrl"] containsString:@"Upload"]) {
            NSString *path = [NSString stringWithFormat:@"%@%@", Ksdby_api_Img, dic[@"ImgUrl"]];
            path = [path stringByReplacingOccurrencesOfString:@"~" withString:@""];
            [btn sd_setBackgroundImageWithURL:[NSURL URLWithString:path] forState:UIControlStateNormal];
        } else {
            NSString *path = [NSString stringWithFormat:@"%@/%@", [FileFunction getSandBoxPath], dic[@"ImgUrl"]];
