...
@implementation XXNet

+ (XXNet *)sharedUtil {
    static dispatch_once_t  onceToken;
    static XXNet * setSharedInstance;
    dispatch_once(&onceToken, ^{
        setSharedInstance = [[XXNet alloc] init];            });
...
+(void)GetURL:(NSString *)URLString
   ...
   parameters:(NSMutableDictionary*)parameters
    ...
    AFHTTPSessionManager *manager= [AFAppDotNetAPIClient manager];//sharedClient_token];
// 2.申明返回的结果是二进制类型
...
// 4.请求超时，时间设置
    ...
    [manager GET:URLString parameters:parameters progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        succeed([XXNet dictionaryWithJsonString:responseStr]);
...
+(void)requestAFURL:(NSString *)URLString
         parameters:(id)parameters
          imageData:(NSData *)imageData
            ...
            failure:(void (^)(NSError *error))failure{
    // 0.设置API地址
    URLString = [NSString stringWithFormat:@"%@%@",Ksdby_api_2,[URLString stringByReplacingOccurrencesOfString:@" " withString:@"%20"]];
    ...
    [manager POST:URLString  parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        // 设置时间格式
        NSString *str = [formatter stringFromDate:[NSDate date]];        NSString *fileName = [NSString stringWithFormat:@"%@.png", str];
