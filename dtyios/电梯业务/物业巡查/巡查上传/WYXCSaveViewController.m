...
@interface WYXCSaveViewController ()
{
    UILabel *dtWeiZhi_2;
...
@implementation WYXCSaveViewController
...
-(void)getSchoolCourse
{
    ...
    NSArray *StepNameArr = [_allTags valueForKey:@"StepName"];
    NSArray *IsTakePhotos = [_allTags valueForKey:@"IsTakePhoto"];
    ...
    for (NSDictionary * dic in _allTags) {
...
-(void)initWindow{
    ...
    UILabel *dtWeiBao = [MyControl createLabelWithFrame:CGRectMake(40, 10, 80, 20) Font:14.0 Text:@"上次巡查:"];
    ...
    UIView *viewlab=[MyControl createViewWithFrame:CGRectMake(0, 220, bounds_width.size.width,50) backColor:[UIColor whiteColor]];
    [headview addSubview:viewlab];
    
    UIImageView *img_headlab=[MyControl createImageViewWithFrame:CGRectMake(10, 10, 20, 20) imageName:@"wyxc_detail"];
    [viewlab addSubview:img_headlab];
    
    
    UILabel *lab = [MyControl createLabelWithFrame:CGRectMake(40, 10, 80, 20) Font:14 Text:@"物业巡查"];
    [viewlab addSubview:lab];
...
-(void)commitButtonClick{
    ...
    for (int i=0; i< _sourceArray.count; i++) {
        ...
        if([type2 isEqual:@"1"]|| [selectBHE isEqual:@"1"] )
        {
            ...
            if(havePic==0)
            {
                [CommonUseClass showAlter:[NSString stringWithFormat:@"巡查步骤%d请拍照",i+1]];
                return;
            }
        }
    }
...
-(NSString *)getStrUpdate{
    WYXCClass *ewm_class = [[WYXCClass alloc]init];
    
    NSMutableArray *mutableArr = [[NSMutableArray alloc]init];
    
    
    for (int i = 0; i < _sourceArray.count; i ++) {
        //ewm_class.PropertyCheckId =_PropertyCheckId;// @"0";
        //ewm_class.CheckDetailId = @"0";
        //ewm_class.CheckId = @"0";
        
        _UploadDate=[CommonUseClass FormatString:_UploadDate];
        if(![@"尚未巡查" isEqual:_UploadDate])
        {
            ewm_class.CheckDate =_UploadDate;//[self getCurrentTimes];
        }
...
{
    NSString *currUrl2=@"PropertyCheck/SaveCheckDetail";
    [HTTPSessionManager
     post:currUrl2
     parameters:parm
...
- (void)cc{
    ...
    for (UIViewController *temp in self.navigationController.viewControllers) {
        if ([temp isKindOfClass:[WYXCListWebViewController class]]) {
...
- (NSString *)addPic:(NSString *)currNum forAll:(NSString *)strstr
{
    if (arr_Photos.count>0) {
        ...
        {
            NSString *str_0=@"\"PhotoUrl\" :\"";
            str_0=[str_0 stringByAppendingString:currImg];
            str_0=[str_0 stringByAppendingString:@"\","];
            
            NSString *d = strstr;
            strstr = [d stringByReplacingOccurrencesOfString:@"\"PhotoUrl\" : null," withString:str_0];
        }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([dict[@"type"] intValue] == SZQuestionOpenQuestion) {
        ...
        return title_height + option_height + topDistance + 50;//40
    }
