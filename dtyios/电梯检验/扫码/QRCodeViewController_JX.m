...
@interface QRCodeViewController_JX ()<AVCaptureMetadataOutputObjectsDelegate>
{
    float jing;
    float  wei;
    NSString *addressStr;
...
@implementation QRCodeViewController_JX
@synthesize app;
...
-(void)gotoDetail
{
    ...
    NSString * liftstr = [LiftNum stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    //GetIsInspectByLiftNum
    [dicHeader setValue:liftstr forKey:@"LiftNum"];
    [dicHeader setValue:app.userInfo.UserID forKey:@"UserId"];
   
    
//    [dicHeader setValue:app.userInfo.UserID forKey:@"UserId"];
     NSString *string=[NSString stringWithFormat:@"Inspect/GetInspectType?LiftNum=%@",[NSString stringWithFormat:@"%@",liftstr] ];
    [XXNet GetURL:string header:dicHeader parameters:nil succeed:^(NSDictionary *data) {
        ...
        else if ([[CommonUseClass FormatString: data[@"Message"]] isEqual:@"任务不存在"]) {
            [self outLine];
        }
        else
        {
