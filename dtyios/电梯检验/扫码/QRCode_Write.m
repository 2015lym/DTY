...
@interface QRCode_Write ()

@end

@implementation QRCode_Write
...
- (void)gotoDetail
{
    ...
    NSString *liftstr=[CommonUseClass FormatString:_liftId.text];
   liftstr = [liftstr stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    
    [[AFAppDotNetAPIClient sharedClient_token].requestSerializer setValue:[NSString stringWithFormat:@"%@",liftstr] forHTTPHeaderField:@"LiftNum"];
    [[AFAppDotNetAPIClient sharedClient].requestSerializer setValue:[NSString stringWithFormat:@"%@",app.userInfo.UserID] forHTTPHeaderField:@"UserId"];
     ...
     NSString *string=[NSString stringWithFormat:@"Inspect/GetInspectType?LiftNum=%@&UserId=%@",[NSString stringWithFormat:@"%@",liftstr],app.userInfo.UserID ];
    [[AFAppDotNetAPIClient sharedClient_token]
     ...
     success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
         ...
         if (dic_result.count>0) {
             ...
             else if ([[CommonUseClass FormatString: dic_result[@"Message"]] isEqual:@"任务不存在"]) {
                 [self outLine];
             }
             else
             {
                [CommonUseClass showAlter:dic_result[@"Message"]];
             }
         }
         else
         {
...
- (void)gotoDetail_go{
    JYTypeViewController *ctvc=[[JYTypeViewController alloc] init];
    ctvc.liftNum=_liftId.text;
    [self.navigationController pushViewController:ctvc animated:YES];
}
